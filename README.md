# Pixiu-RLS

<div align="center">

**基于 Go + Redis 的高性能分布式限流配额服�?*

[![Go Version](https://img.shields.io/badge/Go-1.21+-00ADD8?style=flat&logo=go)](https://golang.org)
[![Redis](https://img.shields.io/badge/Redis-7.0+-DC382D?style=flat&logo=redis&logoColor=white)](https://redis.io)
[![License](https://img.shields.io/badge/License-MIT-green.svg)](LICENSE)

支持多种限流算法和多维度控制，适用于网关和微服务架�?

[特性](#核心功能) �?[快速开始](#快速开�? �?[文档](#文档) �?[性能](#性能指标)

</div>

---

## 📖 项目简�?

Pixiu-RLS (Rate Limiting Service) 是一�?*轻量级但功能强大**的分布式限流服务，专为分布式系统设计，提供统一的流量控制能力�?

### 亮点特�?

- 🚀 **极致性能**：基�?RCU 快照机制，规则查询性能提升 **60+ �?*
- 🔧 **灵活配置**：支持令牌桶算法、多维度限制、配额管�?- 🔥 **热更�?*：规则动态更新，无需重启服务
- 🛡�?**自我保护**：熔断机制自动保护系�?
- 📊 **精确限流**：基�?Redis + Lua 实现分布式精确限�?
- 🎯 **易于集成**：RESTful API，支持任何编程语言接入

## �?核心功能

### 限流算法

- �?**令牌�?*（Token Bucket）：支持流量突发，平滑限�?
### 控制维度

- 🔹 **多维度组�?*：IP、用�?ID、应�?ID、路由、设�?ID 等任意组�?
- 🔹 **多级配额**：分钟级、小时级、天级配额限�?
- 🔹 **黑白名单**：灵活配置豁免或永久限制名单

### 高级特�?

- 🛡�?**熔断保护**：自动熔断过载保护，避免雪崩
- 🔄 **规则热更�?*：通过 API �?Redis Pub/Sub 动态更新规�?
- 📈 **高性能缓存**：RCU 无锁快照，支持百万级并发
- 🌐 **分布式一�?*：基�?Redis 保证多实例一致�?

## 🏗�?架构概览

```
┌─────────────────────────────────────────────────────────────�?
�?                     HTTP API Layer                         �?
�?             (RESTful API / Health Check)                   �?
└────────────────────┬────────────────────────────────────────�?
                     �?
                     �?
┌─────────────────────────────────────────────────────────────�?
�?                  Rate Limit Engine                         �?
�? ┌──────────────────────────────────────────────────────�?  �?
�? �? �?黑白名单检�? �?维度哈希  �?配额检�? �?限流执行  �?  �?�? └──────────────────────────────────────────────────────�?  �?
└────────────┬───────────────────────────────┬────────────────�?
             �?                              �?
             �?                              �?
┌─────────────────────────�?    ┌──────────────────────────────�?
�?  RCU Snapshot Cache    �?    �?  Limiter Implementation   �?
�?  (Rules Management)    �?    �?  �?           �?
�?  �?无锁读取 (60x 性能) �?    �?  �?Token Bucket             �?
�?  �?热更新支�?          �?    �?  �?             �?
└─────────────────────────�?    └──────────────────────────────�?
             �?
             �?
┌─────────────────────────────────────────────────────────────�?
�?                     Redis Cluster                          �?
�? �?规则存储  �?限流状�? �?Pub/Sub 更新通知                 �?
└─────────────────────────────────────────────────────────────�?
```

### 核心模块

| 模块 | 职责 | 关键技�?|
|------|------|---------|
| `api` | HTTP 接口�?| Gorilla Mux, RESTful API |
| `core` | 限流引擎 | 策略模式, 依赖注入 |
| `rcu` | 高性能缓存 | **RCU 无锁快照�?0x 性能提升�?* |
| `limiter` | 限流算法 | Lua 脚本, Redis 原子操作 |
| `repo` | 数据访问�?| Redis 客户端封�?|
| `rules` | 规则管理 | 热更�? Pub/Sub |
| `config` | 配置管理 | YAML 配置 |
| `util` | 工具函数 | 哈希, 维度处理 |

## 🚀 快速开�?

### 环境要求

- **Go**: 1.25 
- **Redis**: 7.0+
- **操作系统**: Linux / macOS / Windows

### 安装部署

#### 1. 克隆项目

```bash
git clone https://github.com/your-org/pixiu-rls.git
cd pixiu-rls
```

#### 2. 配置文件

编辑 `configs/rls.yaml`:

```yaml
server:
  httpAddr: ":8080"

redis:
  mode: "standalone"  # standalone | sentinel | cluster
  addr: "localhost:6379"
  db: 0
  prefix: "pixiu:rls"
  updatesChannel: "pixiu:rls:updates"

features:
  audit: "none"
  localFallback: false

bootstrapRules:
  - ruleId: "api-default"
    match: "/api/*"
    algo: "token_bucket"
    windowMs: 1000
    limit: 100
    burst: 20
    dims: ["ip", "route"]
    enabled: true
    quota:
      perMinute: 1000
      perHour: 10000
      perDay: 100000
```

#### 3. 启动 Redis

```bash
# 使用 Docker
docker run -d -p 6379:6379 --name redis redis:7-alpine

# 或使�?docker-compose
docker-compose -f deployments/docker-compose.yaml up -d
```

#### 4. 运行服务

```bash
# 编译
go build -o rls-http ./cmd/rls-http

# 运行
./rls-http -c configs/rls.yaml
```

服务启动后访问：`http://localhost:8080`

### 快速测�?

#### 1. 健康检�?

```bash
curl http://localhost:8080/health
```

#### 2. 限流检�?

```bash
curl -X POST http://localhost:8080/api/check \
  -H "Content-Type: application/json" \
  -d '{
    "ruleId": "api-default",
    "dims": {
      "ip": "192.168.1.1",
      "route": "/api/login"
    }
  }'
```

响应示例�?

```json
{
  "allowed": true,
  "remaining": 95,
  "retryAfterMs": 0,
  "reason": "allowed"
}
```

#### 3. 规则管理

```bash
# 获取所有规�?
curl http://localhost:8080/api/rules

# 创建/更新规则
curl -X POST http://localhost:8080/api/rules \
  -H "Content-Type: application/json" \
  -d '{
    "ruleId": "api-login",
    "match": "/api/login",
    "algo": "token_bucket",
    "windowMs": 1000,
    "limit": 10,
    "burst": 5,
    "dims": ["ip"],
    "enabled": true
  }'
```

## 📊 性能指标

### RCU 快照性能

基于 Intel i7-13650HX 的基准测试：

| 操作 | QPS | 延迟 | vs RWMutex |
|------|-----|------|-----------|
| **规则查询** | ~31 �?s | **0.03 ns** | **62x** �?|
| **规则更新** | ~5200 �?s | 21.89 ns | 2x |
| **混合读写�?0%读）** | ~1.8 �?s | 5.59 ns | 30x |

### 限流性能

- **��ʵ�� QPS**: 10��+
- **延迟**: P99 < 5ms
- **并发连接**: 支持 10000+ 并发

### 资源占用

- **内存**: ~50MB�?000条规则）
- **CPU**: 单核 < 10%�?�?QPS�?
- **Redis**: Key 数量取决于维度组合数

## 📚 文档

### 核心文档

- [快速入门](docs/RCU_QUICKSTART.md) - RCU 快照快速入�?
- [API 文档](docs/API.md) - 完整�?API 接口说明
- [部署指南](docs/DEPLOYMENT.md) - 生产环境部署最佳实�?
- [开发指南](docs/DEVELOPMENT.md) - 开发者指南和贡献说明
- [本地降级说明](docs/OPTIMIZATION_LOCAL_FALLBACK.md) - Redis 故障时本地限�?配额降级说明

### 技术文�?

- [RCU 集成说明](docs/RCU_INTEGRATION.md) - RCU 快照机制详解
- [架构设计](docs/RCU_ARCHITECTURE.md) - 系统架构和设计思路
- [RCU 原理](internal/rcu/README.md) - RCU 无锁快照原理

### 示例代码

- [RCU 使用示例](examples/rcu_example.go) - RCU 快照的使用示�?

## 🧪 测试

### 运行所有测�?

```bash
go test ./...
```

### 运行特定模块测试

```bash
# RCU 快照测试
go test -v ./internal/rcu/

# 规则缓存测试
go test -v ./internal/rules/

# 工具函数测试
go test -v ./internal/util/

# 核心引擎测试
go test -v ./internal/core/
```

### 性能基准测试

```bash
# RCU 性能测试
cd internal/rcu
go test -bench=. -benchmem -benchtime=1s

# 规则缓存性能测试
cd internal/rules
go test -bench=. -benchmem
```

### 测试覆盖�?

```bash
go test -cover ./...
```

## 🔧 配置说明

### 完整配置示例

```yaml
server:
  httpAddr: ":8080"  # HTTP 监听地址

redis:
  mode: "standalone"  # standalone | sentinel | cluster
  addr: "localhost:6379"      # Redis 地址
  db: 0                       # 数据库编�?
  prefix: "pixiu:rls"         # Key 前缀
  updatesChannel: "pixiu:rls:updates"  # Pub/Sub 频道

features:
  audit: "none"               # 审计: none | redis_stream
  localFallback: false  # Redis 故障时是否本地降级（单机内存，非一致）

bootstrapRules:  # 启动时加载的规则
  - ruleId: "example"
    match: "/api/*"           # 路由匹配
    algo: "token_bucket"    # �㷨��token_bucket
    windowMs: 1000            # 时间窗口（毫秒）
    limit: 100                # 速率限制
    burst: 20                 # 突发容量
    dims: ["ip", "route"]     # 限流维度
    enabled: true             # 是否启用
    quota:                    # 配额限制
      perMinute: 1000
      perHour: 10000
      perDay: 100000
    breaker:                  # 熔断配置
      enabled: true
      rlDenyThreshold: 20
      rlDenyWindowMs: 10000
      minOpenMs: 8000
```

## 🛠�?开�?

### 项目结构

```
Pixiu-RLS/
├── cmd/                    # 应用入口
�?  └── rls-http/          # HTTP 服务
├── configs/               # 配置文件
├── deployments/           # 部署配置
├── docs/                  # 文档
├── examples/              # 示例代码
├── internal/              # 内部�?
�?  ├── api/              # API �?
�?  ├── config/           # 配置管理
�?  ├── core/             # 核心引擎
�?  �?  └── limiter/     # 限流策略
�?  ├── rcu/              # RCU 快照
�?  ├── repo/             # Redis 仓库
�?  ├── rules/            # 规则管理
�?  ├── types/            # 类型定义
�?  └── util/             # 工具函数
└── README.md
```

### 本地开�?

```bash
# 安装依赖
go mod download

# 运行测试
go test ./...

# 启动服务（开发模式）
go run ./cmd/rls-http -c configs/rls.yaml

# 代码格式�?
go fmt ./...

# 静态检�?
go vet ./...
```

### 添加新的限流策略

1. �?`internal/core/limiter/` 创建新文�?
2. 实现 `core.Strategy` 接口
3. �?`cmd/rls-http/main.go` 注册策略
4. 添加对应的测试文�?

示例�?

```go
type MyLimiter struct {
    repo *repo.RedisRepo
}

func (l *MyLimiter) Allow(ctx context.Context, rule config.Rule, key string, now time.Time) (types.Decision, error) {
    // 实现你的限流逻辑
    return types.Decision{Allowed: true}, nil
}
```

## 🤝 贡献

欢迎贡献！请查看 [开发指南](docs/DEVELOPMENT.md)�?

### 贡献流程

1. Fork 本仓�?
2. 创建特性分�?(`git checkout -b feature/AmazingFeature`)
3. 提交更改 (`git commit -m 'Add some AmazingFeature'`)
4. 推送到分支 (`git push origin feature/AmazingFeature`)
5. 开�?Pull Request

## 📝 许可�?

本项目采�?MIT 许可�?- 查看 [LICENSE](LICENSE) 文件了解详情�?

## 🙏 致谢

- [Redis](https://redis.io/) - 高性能内存数据�?
- [Gorilla Mux](https://github.com/gorilla/mux) - HTTP 路由�?
- [Go Redis](https://github.com/redis/go-redis) - Redis Go 客户�?

## 📞 联系方式

- 项目主页: https://github.com/your-org/pixiu-rls
- 问题反馈: https://github.com/your-org/pixiu-rls/issues

---

<div align="center">
Made with ❤️ by Pixiu-RLS Team
</div>

